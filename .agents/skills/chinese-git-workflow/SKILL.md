...
description: 國內 Git 平臺配置參考——Gitee、Coding.net、極狐 GitLab、CNB 的 SSH/HTTPS/憑據/CI 接入差異與映象同步配置。僅在使用者顯式 /chinese-git-workflow 時呼叫，不要根據脈絡 (Context)自動觸發。
version: "1.0.0"
license: MIT
metadata:
  hermes:
...
國內團隊用 Git 經常踩的坑：GitHub 訪問不穩定、CI/CD 方案照搬國外水土不服、commit message 中英混雜沒有規範。本技能提供一套**完整適配國內平臺和團隊習慣的 Git 工作流**。
...
# HTTPS 認證：使用者名稱固定為 cnb，密碼為個人訪問令牌（Access Token）
# 在 CNB 平臺 → 個人設定 → 訪問令牌 中生成
...
**適合：** 中大團隊、版本釋出節奏固定（如雙週迭代）、需要維護多個版本。
...
- `release/*` — 釋出分支，從 develop 拉出，只修 bug 不加功能
- `feat/*` — 功能分支
...
**規則：**
- `main` 分支受保護，只能通過 PR/MR 合併
- `dev` 分支對應測試環境，自動部署
- 功能分支從 `dev` 拉出，合回 `dev`
- `dev` 測試通過後，合併到 `main` 進行釋出
...
1. 全部小寫，用 `-` 連線單詞（不用下劃線或駝峰）
2. 字首明確分支型別：`feat/`、`fix/`、`hotfix/`、`release/`
3. 關聯任務管理平臺的編號（如有）：`feat/TAPD-12345-description`
4. 長度適中，能看出分支目的即可
...
方案：升級 SDK 至 8.0.38，並更新 Associated Domains 配置。
...
```yaml
# .cnb.yml — branch-first 結構，直接指定 Docker 映象跑流水線
main:
  push:
    - docker:
        image: node:20
...
| 金鑰管理 | 環境變數配置 | 憑據管理 | CI/CD Variables | Access Token |
