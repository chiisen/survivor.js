...
**個人技能存放在智慧體特定的目錄中（Claude Code 用 `~/.claude/skills`，Codex 用 `~/.agents/skills/`）**

你編寫測試案例（帶子智慧體的壓力場景），觀察它們失敗（基線行為），編寫技能（文件），觀察測試通過（智慧體遵守規則），然後重構（堵住漏洞）。

**核心原則：** 如果你沒有觀察到智慧體在沒有該技能時失敗，你就不知道這個技能是否教了正確的東西。

**必需背景：** 在使用此技能前，你必須理解 superpowers:test-driven-development。該技能定義了基本的紅-綠-重構迴圈。本技能將 TDD 適配到文件編寫中。

**官方指南：** Anthropic 官方的技能編寫最佳實踐請參見 anthropic-best-practices.md。該文件提供了補充本技能 TDD 導向方法的額外模式和指南。

## 什麼是技能？

**技能**是經過驗證的技術、模式或工具的參考指南。技能幫助未來的 Claude 例項找到並應用有效的方法。
...
| **測試案例** | 帶子智慧體的壓力場景 |
| **生產程式碼** | 技能文件（SKILL.md） |
| **測試失敗（紅）** | 智慧體在沒有技能時違反規則（基線） |
| **測試通過（綠）** | 智慧體在有技能時遵守規則 |
| **重構** | 在保持合規的同時堵住漏洞 |
| **先寫測試** | 在編寫技能之前先執行基線場景 |
| **觀察失敗** | 記錄智慧體使用的確切合理化藉口 |
| **最小程式碼** | 編寫針對那些具體違規行為的技能 |
| **觀察通過** | 驗證智慧體現在遵守規則 |
| **重構迴圈** | 發現新的合理化藉口 → 堵住 → 重新驗證 |
...
- `description`：第三人稱，僅描述何時使用（不是做什麼）
  ...
  - 包含具體的症狀、場景和脈絡 (Context)
  - **絕不總結技能的流程或工作流**（參見 CSO 章節瞭解原因）
  - 儘量控制在 500 字元以內
...
**為什麼這很重要：** 測試表明，當描述總結了技能的工作流時，Claude 可能會跟隨描述而非閱讀完整的技能內容。一個寫著"任務間進行程式碼審查"的描述導致 Claude 只做了一次審查，儘管技能的流程圖清楚地展示了兩次審查（先規格合規再程式碼質量）。
...
使用 Claude 會搜尋的詞語：
- 錯誤資訊："Hook timed out"、"ENOTEMPTY"、"race condition"
...
**將細節移到工具幫助中：**
```bash
# 錯誤：在 SKILL.md 中列出所有引數
search-conversations supports --text, --both, --after DATE, --before DATE, --limit N

# 正確：引用 --help
search-conversations 支援多種模式和過濾器。執行 --help 檢視詳情。
```
...
# 正確：引用其他技能
始終使用子智慧體（節省 50-100 倍脈絡 (Context)）。必需：使用 [other-skill-name] 工作流。
```
...
[用搜索查詢分派子智慧體："React Router authentication error handling 401"]
...
- ❌ 差的：`@skills/testing/test-driven-development/SKILL.md`（強制載入，浪費脈絡 (Context)）

**為什麼不用 @ 連結：** `@` 語法會立即強制載入檔案，在你需要之前就消耗 200k+ 的脈絡 (Context)。
...
**必需背景：** superpowers:test-driven-development 技能解釋了為什麼這很重要。相同的原則適用於文件。
...
執行紀律的技能（如 TDD）需要抵抗合理化。智慧體很聰明，在壓力下會找到漏洞。

**心理學說明：** 理解說服技巧為什麼有效有助於你係統性地應用它們。參見 persuasion-principles.md 瞭解研究基礎（Cialdini, 2021; Meincke et al., 2025），涵蓋權威、承諾、稀缺、社會認同和歸屬原則。
...
從基線測試中捕獲合理化藉口（參見下方測試章節）。智慧體使用的每個藉口都進入表中：
...
這就是"觀察測試失敗"——在編寫技能之前你必須看到智慧體自然會怎麼做。

### 綠：編寫最小技能

編寫針對那些具體合理化藉口的技能。不要為假設情況新增額外內容。

用技能執行相同的場景。智慧體應該現在遵守。

### 重構：堵住漏洞

智慧體找到了新的合理化藉口？新增明確的反駁。重新測試直到無懈可擊。
...
同樣的鐵律：沒有失敗的測試就不寫技能。
同樣的迴圈：紅（基線）→ 綠（寫技能）→ 重構（堵漏洞）。
同樣的好處：更高的質量、更少的意外、無懈可擊的結果。
