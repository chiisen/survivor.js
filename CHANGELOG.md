...
- **Phase 3 JSDoc 型別標註 (objectPool.js)**：由 Pi 執行，為 `js/objectPool.js` 補上 `// @ts-check` 與 class 層級 + `constructor` / `get` / `release` 等方法的 JSDoc 型別標註，含進階函式型別 `() => object`、`(obj, ...args) => void` 標註；順手清掉原檔空白行尾。`npm test` 73/73 全綠無回歸。
- **Phase 4 JSDoc 型別標註 (talent.js)**：由 pi-mono (npm 最新版 0.80.6) 透過 `npx @earendil-works/pi-coding-agent -p --approve` 派發執行，為 `js/talent.js` 補上 `// @ts-check`、`@typedef Upgrade` 完整物件型別定義 (5 個 @property)、`UPGRADES` 用 `@type {Upgrade[]}` 標註、`getRandomUpgrades` 的 `@param {number} [count=3]` 與 `@returns` 型別標註。`npm test` 73/73 全綠無回歸。本次派發證實本機舊版 `pi` 0.80.3 不穩定 (5 次測試僅 1 次成功),改用 `npx` 跑最新版 0.80.6 一發成功。
